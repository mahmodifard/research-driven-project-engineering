...
Read [references/north-star-review.md](references/north-star-review.md) after the product North Star is established. Freeze each approved North Star as an immutable, provenance-backed `NSV-*` constitution version and record activation/supersession with append-only `NSE-*` events; never rewrite an approved version. At required material checkpoints, use a dedicated read-only `north_star_reviewer` subagent when delegation is available and authorized; otherwise run the same questions as a labeled local self-check. Every review must pin the exact North Star version effective at review time. The reviewer challenges product alignment, consumability, integration conflict, proportional complexity, and adoption friction. It is advisory, cannot mutate state, and cannot declare a gate blocked. Record accepted outputs in `north-star-review.yaml`; never represent a self-check as independent review.
